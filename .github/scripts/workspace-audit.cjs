...
async function navigation(page,lang,width){
 await page.locator('.workspace-nav [data-page-link="capabilities"]').click();assert.equal(await page.locator('.capability:visible').count(),1);
 ...
 await search.fill('NetCDF');assert.ok(await page.locator('[data-feature-link="netcdf"]').isVisible());await page.locator('[data-feature-link="netcdf"]').click();assert.ok(page.url().endsWith('#cap-netcdf'));assert.equal(await search.inputValue(),'');
 ...
 await choose(page,data.items.find(i=>i.id==='ga'));await page.locator('#cap-ga .related-capabilities a[href="#cap-runoff"]').click();assert.ok(page.url().endsWith('#cap-runoff'));await page.goBack();assert.ok(page.url().endsWith('#cap-ga'));assert.ok(await page.locator('#cap-ga').isVisible());
 ...
 for(const id of ['programs','research','download','home']){await page.locator('.workspace-nav [data-page-link="'+id+'"]').click();assert.equal(await page.locator('[data-workspace-page]:visible').count(),1);assert.ok(await page.locator('[data-workspace-page="'+id+'"]').isVisible());await bounds(page,lang+' '+width+' '+id);}
