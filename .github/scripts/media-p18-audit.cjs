  ...
  try {
    ...
    for (const lang of ['en','ko']) {
      for (const width of [1440,768,390]) {
        ...
        await page.locator('.workspace-nav a[href="#download"]').click();
        const guide = page.locator('#download a[href="'+(lang==='ko'?'../':'')+'media.html?lang='+lang+'"]');
        assert.equal(await guide.count(),1);
        assert.equal(await guide.innerText(),lang === 'ko' ? '발표자료·영상·이미지 ↗' : 'Presentation, video and images ↗');
        await guide.click();
