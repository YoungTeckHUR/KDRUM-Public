  ...
  try {
    ...
    for (const lang of ['en','ko']) {
      for (const width of [1440,768,390]) {
        ...
        const introVideo = page.locator('.home-introduction video');
        assert.ok(await introVideo.isVisible());
        assert.equal(await introVideo.getAttribute('autoplay'),null);
        await introVideo.evaluate(async video => {video.muted=true;await video.play();});
        await page.waitForFunction(()=>{const v=document.querySelector('.home-introduction video');return v.currentTime>1&&v.videoWidth>0&&!v.error;});
        assert.ok((await page.locator('.intro-pdf').getAttribute('href')).endsWith(manifest.presentation_pdf));
        await page.locator('.intro-overview').click();
        await page.locator('#overview').waitFor({state:'visible'});
        assert.ok(await introVideo.evaluate(video=>video.paused),'Leaving home pauses the introduction video');
        await page.locator('.workspace-nav a[href="#home"]').click();
        const guide = page.locator('.intro-gallery');
        await guide.waitFor({state:'visible'});
        assert.equal(await guide.count(),1);
        assert.equal(await guide.innerText(),lang === 'ko' ? '영상·전체 소개자료 →' : 'Video & visual guide →');
        await guide.click();
        await page.waitForURL(`**/media.html?lang=${lang}`);
        assert.equal(await page.locator('html').getAttribute('lang'),lang);
        assert.equal(await page.locator('#gallery-'+lang+' .media-card').count(),18);
        assert.equal(await page.locator('#gallery-'+lang+' .media-card .badge').count(),18);
        ...
        for (let i=0;i<18;i++) {
          const button = page.locator('#gallery-'+lang+' .media-card > a').nth(i);
          await button.scrollIntoViewIfNeeded();
          await page.waitForFunction(index => {
            const img = document.querySelectorAll('#gallery-'+document.documentElement.lang+' .thumb')[index];
          ...
          await page.waitForFunction(() => {
            ...
            return document.getElementById('image-viewer').open && img.complete && img.naturalWidth > 0;
          });
          assert.ok((await page.locator('#image-viewer img').getAttribute('src')).endsWith(manifest.feature_images[i]));
          assert.equal(await page.locator('#image-viewer button').getAttribute('aria-label'),lang==='en'?'Close':'닫기');
          if(i===0){const zoom=page.locator('#image-viewer input');await zoom.focus();await zoom.press('ArrowRight');await zoom.press('ArrowRight');assert.equal(await zoom.inputValue(),'2');assert.ok(await page.locator('.image-pan').evaluate(el=>el.scrollWidth>el.clientWidth*1.9));}
          if(i===0) await page.screenshot({path:path.join(output,`${name}-enlarged.png`)});
          if(i%2===0) await page.keyboard.press('Escape');
          else await page.locator('#image-viewer button').click();
          assert.equal(await page.locator('#image-viewer').evaluate(dialog=>dialog.open),false);
        }
        ...
        await page.locator(`[data-media-lang="${other}"]`).click();
        await page.waitForFunction(expected=>document.documentElement.lang===expected&&document.documentElement.dataset.siteReady==='true',other);
        assert.equal(await page.locator('html').getAttribute('lang'),other);
        assert.ok(page.url().endsWith(`?lang=${other}`));
        await page.reload();
        await page.waitForFunction(()=>document.documentElement.dataset.siteReady==='true');
        assert.equal(await page.locator('html').getAttribute('lang'),other);
        await page.locator(`[data-media-lang="${lang}"]`).click();
        await page.waitForFunction(()=>document.documentElement.dataset.siteReady==='true');
        assert.equal(await page.locator('[data-media-home]').getAttribute('href'),lang==='ko'?'ko/':'./');
        ...
        await page.locator('[data-media-home]').click();
        await page.waitForFunction(() => document.documentElement.dataset.siteReady === 'true');
