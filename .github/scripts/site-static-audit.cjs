...
// maturity remain tied to the original public inventory. Prose meaning is
// reviewed separately in HOMEPAGE_TONE_REVIEW.md.
for(const item of old){const now=data.items.find(i=>i.id===item.id);assert.ok(now,item.id);for(const key of ['g','s','kind','v','ko','en'])assert.equal(now[key],item[key],item.id+' '+key);for(const key of ['sumKo','sumEn','nowKo','nowEn'])assert.ok(typeof now[key]==='string'&&now[key].trim(),item.id+' '+key);}
...
console.log('PASS static routes, local assets, original 45 capability identities/statuses/technical labels, bilingual descriptions, separate unpublished viewer, 46 entries, 12 diagrams, one runtime');
